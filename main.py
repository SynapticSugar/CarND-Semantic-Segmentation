...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    # Layer 7 up
    layer7_up = tf.layers.conv2d_transpose(conv7_1x1, num_classes, 4, strides= (2, 2), padding='same', 
        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    # Layer 4 1x1 conv
    conv4_1x1 = tf.layers.conv2d(vgg_layer4_out, num_classes, 1, padding='same', 
        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    # Add Layer 4 add 7
    conv4_add = tf.add(layer7_up, conv4_1x1)
    # Layer 4
    conv4_up = tf.layers.conv2d_transpose(conv4_add, num_classes, 4, strides= (2, 2), padding='same', 
        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    # Layer 3 1x1 conv
    conv3_1x1 = tf.layers.conv2d(vgg_layer3_out, num_classes, 1, padding='same', 
        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    # Layer 3 add 4
    conv3_add = tf.add(conv4_up, conv3_1x1)
    # Layer 3 up
    nn_last_layer = tf.layers.conv2d_transpose(conv3_add, num_classes, 16, strides= (8, 8), padding='same', 
        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
